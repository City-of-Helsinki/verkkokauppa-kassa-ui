...
export const ContractRow: FunctionComponent<Props &
  ...
  const showInvoiceContractRow = orderType !== "subscription" &&
    paymentMethod &&
  ...
  return <ul key={'contract-row'}>
    ...
    {
      showInvoiceContractRow && <li key={'li-summary.contract.invoice-label'}>
        <Trans key={'summary.contract.invoice-label'} i18nKey="summary.contract.invoice-label" t={t}> Teksti <a key={'summary.contract.invoice-label-link'} target="_blank"  href={`${paymentApiUrl}${t('summary.contract.invoice-terms-filename')}`} rel="noreferrer">Linkki</a></Trans>
      </li>
    }
